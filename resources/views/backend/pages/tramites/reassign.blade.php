...
@section('title')
    {{ __('Reasignar Trámites - Panel de Reasignación') }}
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">{{ __('Reasignar Trámites') }}</h4>
                <ul class="breadcrumbs pull-left">
                    <li><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
                    <li><span>{{ __('Reasignar Trámites') }}</span></li>
                </ul>
            </div>
        </div>
    ...
    <div class="row">
                            ...
                            <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion">
                                <div class="card-body">
                                    ...
                                    <p class="float-right mb-2" style="padding: 5px;">
                                        @if (auth()->user()->can('tramite.reassign'))
                                            <button id="reasignarTramites" class="btn btn-success" type="button" data-toggle="modal" data-target="#modalReasignar">Reasignar trámites</button>
                                        @endif
                                    </p>
        ...
        <div class="modal fade" id="modalReasignar" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Reasignar trámites</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="form-row">
                        <div class="form-group col-md-6 col-sm-12">
                            <label for="funcionario_a_reasignar">Seleccione un Funcionario:</label>
                            <select id="funcionario_a_reasignar" name="funcionario_search" class="form-control selectpicker" data-live-search="true">
                                <option value="">Seleccione un Funcionario</option>
                                @foreach ($funcionarios as $key => $value)
                                    <option value="{{ $value->id }}">{{ $value->name }}</option>
                                @endforeach
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            <label for="contenido_html" class="form-label">Comentario Reasignación</label>
                            <textarea id="comentario_reasignacion" name="comentario_reasignacion" class="form-control @error('comentario_reasignacion') is-invalid @enderror" rows="5" cols="40" required></textarea>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
                    <button type="button" id="reasignar" class="btn btn-warning" >Reasignar</button>
                </div>
                </div>
            </div>
        </div>
     ...
     <script>
        ...
        $(document).ready(function() {
            ...
            $("#reasignar").on( "click", function() {
                ...
                let funcionario_a_reasignar = $('#funcionario_a_reasignar').val();
                let comentario_reasignacion = $('#comentario_reasignacion').val();
                $("#overlay").fadeIn(300);

                $.ajax({
                    url: "{{ url('admin') }}/tramites/reasignarTramites",
                    method: "POST",
                    data: {
                        proceso_id: proceso_id,
                        tramites_ids: JSON.stringify(selected_table_items),
                        funcionario_a_reasignar: funcionario_a_reasignar,
                    ...
                    success: function (response) {
                        ...
                        $('#success_alert .message').html(response.message);
                        $('#success_alert').show();
                    ...
                    error: function(jqXHR, textoEstado, errorEncontrado) {
                        $("#overlay").fadeOut(300);
                        $('#modalReasignar').modal('hide');
                        $('#error_alert .message').html('Error en la solicitud, por favor vuelva a intentar.');
                        $('#error_alert').show();
                    }
                });
            ...
            $("#reasignarTramites").prop("disabled", true);
        ...
        function loadDataTable(){
            $.ajax({
                url: "{{url('/getTramitesParaReasignarByFilters')}}",
                method: "POST",
                data: {
                ...
                success: function (response) {
                    ...
                    for (let tramite of tramites) {
                        ...
                        htmlReassign +=@if (auth()->user()->can('tramite.reassign')) '<a class="icon-margin" title="Reasignar" style="color: #007bff; cursor:pointer;" onclick="javascript:void(0);mostrarModalResignacion('+ tramite.id +')"><i class="fa fa-exchange fa-2x"></i></a>' @else '' @endif;
        ...
        function mostrarModalResignacion(tramite_id){
            let checkboxes = document.querySelectorAll('#dataTable input[type="checkbox"]');
            selected_table_items = [];
            for (let i = 0; i < checkboxes.length; i++) {
                if(checkboxes[i].id == tramite_id){
        ...
        function toggle(id) {
            ...
            if(selected_table_items.length == 0){
                ...
                $("#reasignarTramites").prop("disabled", false);
            }
        }
