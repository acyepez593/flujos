    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">{{ __('Catálogo') }}</h4>
                <ul class="breadcrumbs pull-left">
                    <li><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
                    <li><span>{{ __('Todos los Catálogos') }}</span></li>
                </ul>
            </div>
        </div>
                ...
                <div class="accordion" id="accordion">
                                ...
                                <div class="card-body">
                                    <form>
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-6 col-sm-12">
                                                <label for="tipo_catalogo_id_search">Buscar por Tipo Catálogo:</label>
                                                <select id="tipo_catalogo_id_search" name="tipo_catalogo_id_search" class="form-control selectpicker" data-live-search="true" multiple>
                                        ...
                                        <button type="button" id="buscarCatalogos" class="btn btn-primary mt-4 pr-4 pl-4">Buscar</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            ...
                            <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion">
                                <div class="card-body">
                                    <h4 class="header-title float-left">{{ __('Catálogos') }}</h4>
                                    <p class="float-right mb-2" style="padding: 5px;">
                                        @if (auth()->user()->can('catalogo.create'))
                                            <a class="btn btn-primary text-white" href="{{ route('admin.catalogos.create') }}">
                                                {{ __('Crear Nuevo') }}
                                            </a>
                                        @endif
                                    </p>
...
@section('scripts')
     ...
     <script>
        ...
        $(document).ready(function() {
            ...
            $( "#buscarCatalogos" ).on( "click", function() {
                $("#overlay").fadeIn(300);
                $('#dataTable').empty();
        ...
        function loadDataTable(){
            $.ajax({
                url: "{{url('/getCatalogosByFilters')}}",
                method: "POST",
                data: {
                    nombre_search: $('#nombre_search').val(),
                    tipo_catalogo_id_search: JSON.stringify($('#tipo_catalogo_id_search').val()),
                ...
                success: function (response) {
                    ...
                    for (let catalogo of catalogos) {
                        ...
                        let rutaEdit = "{{url()->current()}}"+"/"+catalogo.id+"/edit";
                        let rutaDelete = "{{url()->current()}}"+"/"+catalogo.id;
                        let innerHTML = "";
                        let htmlEdit = "";
                        let htmlDelete = "";
                        ...
                        htmlDelete += @if (auth()->user()->can('catalogo.delete')) '<a class="icon-margin" title="Borrar" href="javascript:void(0);" onclick="event.preventDefault(); deleteDialog('+catalogo.id+')"><i class="fa fa-trash fa-2x"></i></a> <form id="delete-form-'+catalogo.id+'" action="'+rutaDelete+'" method="POST" style="display: none;">@method('DELETE')@csrf</form>' @else '' @endif;
