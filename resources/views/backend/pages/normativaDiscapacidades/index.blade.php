...
@section('title')
    {{ __('Normativa Discapacidad - Panel de Normativa Discapacidad') }}
@endsection
                ...
                <div class="accordion" id="accordion">
                            ...
                            <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordion">
                                <div class="card-body">
                                    <form>
                                        ...
                                        <button type="button" id="buscarNormativaDiscapacidades" class="btn btn-primary mt-4 pr-4 pl-4">Buscar</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            ...
                            <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion">
                                <div class="card-body">
                                    <h4 class="header-title float-left">{{ __('Normativa Discapacidades') }}</h4>
                                    <p class="float-right mb-2" style="padding: 5px;">
                                        @if (auth()->user()->can('catalogo.create'))
                                            <a class="btn btn-primary text-white" href="{{ route('admin.normativaDiscapacidades.create') }}">
                                                {{ __('Crear Nuevo') }}
                                            </a>
                                        @endif
                                    </p>
...
@section('scripts')
     ...
     <script>
        ...
        let tableRef = "";
        let tableHeaderRef = "";
        let normativaDiscapacidades = [];
        let creadores = [];

        $(document).ready(function() {

            $( "#buscarNormativaDiscapacidades" ).on( "click", function() {
                $("#overlay").fadeIn(300);
                $('#dataTable').empty();
        ...
        function loadDataTable(){
            $.ajax({
                url: "{{url('/getNormativaDiscapacidadesByFilters')}}",
                method: "POST",
                data: {
                ...
                success: function (response) {
                    ...
                    normativaDiscapacidades = response.normativaDiscapacidades;
                    ...
                    let contador = 1;
                    for (let normativaDiscapacidad of normativaDiscapacidades) {
                        
                        let rutaEdit = "{{url()->current()}}"+"/"+normativaDiscapacidad.id+"/edit";
                        let rutaDelete = "{{url()->current()}}"+"/"+normativaDiscapacidad.id;
                        let innerHTML = "";
                        let htmlEdit = "";
                        let htmlDelete = "";
                        ...
                        innerHTML += 
                            ...
                            if(normativaDiscapacidad.esCreadorRegistro){
                ...
                buttons: {
                    confirm: function () {
                        ...
                        $.ajax({
                            ...
                            dataType: 'json',
                            success: function (response) {
                                $( "#buscarNormativaDiscapacidades" ).trigger( "click" );
                            }
                        });
                    },
                    cancel: function () {
